...
if __name__ == '__main__':
    ...
    if args.output_dir:
        ...
        output_dir = "meta_test_results/lenk/"
    if args.A_and_task_dir:
        A_and_task_dir = args.A_and_task_dir
    else:
        A_and_task_dir = "data/lenk/"
    if args.seed:
        seed = int(args.seed)
    else:
        seed = 11
    ...
    X = pickle.load(open('./data/lenk/X.pkl', 'rb'))
    Y = pickle.load(open('./data/lenk/Y.pkl', 'rb'))
    R = pickle.load(open('./data/lenk/R.pkl', 'rb'))
    task_function = pickle.load(open('./data/lenk/task_function.pkl', 'rb'))
    ...
    mse_all = np.zeros((num_trials,5))
    

    for trial in range(num_trials):
        for N2 in range(2,12,2):
            


            #load data
            X2 = pickle.load(open('./data/lenk/' + 'X0_N2_{N2F}_trial_{trialF}.pkl'.format(N2F=N2,trialF=trial),'rb'))
            Y0 = pickle.load(open('./data/lenk/' + 'Y0.pkl','rb'))
            #Z0 = pickle.load(open('./data/school/' + 'Z0_N2_{N2F}_trial_{trialF}.pkl'.format(N2F=N2,trialF=trial),'rb'))
            R = pickle.load(open('./data/lenk/' + 'R0_N2_{N2F}_trial_{trialF}.pkl'.format(N2F=N2,trialF=trial),'rb'))
            ...
            X0eval = pickle.load(open('./data/lenk/X0eval.pkl','rb'))
            R0eval = pickle.load(open('./data/lenk/X0eval.pkl','rb'))
