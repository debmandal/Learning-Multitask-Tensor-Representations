...
def gradient(R, R_indexed, cov_X, B, inner_X_B, lambd, task_function, batch):
    ...
    #cost = R_indexed - np.expand_dims(inner_X_B, axis=1)
...
def batch_grad_descent(d1, d2, T, R, X, Y, cov_X, eta, eps, r, lambd, task_function, iterations=200, sparse=0):
    # Initialize B to a random tensor d1 x d2 x T
    # TODO: Z is ONLY used for the shape!
    (_, factors) = random_cp((d1, d2, T), full=False, rank=r, orthogonal=True, normalise_factors=True)
